...
class TrainerDaySchedule extends Component {
                  ...
                  <div className="card h-100 w-100 overflow-hidden border-white linkHoverDecLess" style={{ borderRadius: '10px' }}>
                    ...
                    <div className="text-white" style={{ backgroundColor: color }}>
                      ...
                      <div className="cardFootClass d-flex flex-wrap justify-content-between align-items-center">
                        <Link to={`/trainer-classes-shedule-details/${_id}`} className="border-bottom border-white my-3 mx-3 linkHoverDecLess">
                          <button type="button" className="btn text-white px-0">{t('View Details')}</button>
                        </Link>
                        <div className="d-flex align-items-center mx-3 my-3">
                          <h1 className="iconv1 iconv1-capacity m-0"> </h1>
                          <div>
                            <small>{t('Total Members')}</small>
                            <p className="SegoeBold m-0">{`${(occupied ? occupied : 0)}/${capacity}`}</p>
                          </div>
                        </div>
                      </div>
...
export default withTranslation()(connect(mapStateToProps)(TrainerDaySchedule))