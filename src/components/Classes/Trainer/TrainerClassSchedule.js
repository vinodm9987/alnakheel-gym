...
import TrainerWeekSchedule from './TrainerWeekSchedule'
import TrainerMonthSchedule from './TrainerMonthSchedule'

class TrainerClassSchedule extends Component {
  ...
  render() {
    ...
    const { t } = this.props
    return (
      <div className="mainPage p-3 TrainerClassSchedule">
        <div className="row">
          ...
          <div className="col-12 d-flex flex-wrap align-items-center justify-content-between py-3">
            <h2 className="px-1 py-2 my-1">{`${monthFullNames[this.state.startDate.getMonth()]} ${this.state.startDate.getFullYear()}`}</h2>
...
function mapStateToProps({ auth: { loggedUser } }) {
  return {
    loggedUser,
  }
}

export default withTranslation()(connect(mapStateToProps)(TrainerClassSchedule))