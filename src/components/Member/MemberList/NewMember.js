...
import { getFirstRegisterMembers } from '../../../actions/member.action'
...
class NewMember extends Component {
  constructor(props) {
    ...
    this.props.dispatch(getFirstRegisterMembers({ search: "", searchFor: "All" }))

  }

  handleFilter(search, searchFor) {
    this.setState({ search, searchFor }, () => {
      window.dispatchWithDebounce(getFirstRegisterMembers)({ search, searchFor })
    });
  }
  ...
  render() {
    const { t } = this.props
    return (
      <div className={this.state.url === '/members/new-members' ? "tab-pane fade show active" : "tab-pane fade"} id="menu1" role="tabpanel">
...
export default withTranslation()(connect(mapStateToProps)(NewMember))
