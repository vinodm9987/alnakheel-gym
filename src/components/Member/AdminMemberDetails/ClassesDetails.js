...
import { startClass } from '../../../actions/bioStar.action'
...
class ClassesDetails extends Component {
  ...
  handleStartClass(memberId, memberClassId, doneFingerAuth) {
    const { t } = this.props
    if (doneFingerAuth) {
      const memberInfo = {
        memberId,
        memberClassId,
        classStartDate: new Date().toISOString(),
      }
      this.props.dispatch(startClass(memberInfo))
    } else {
      this.props.dispatch({ type: GET_ALERT_ERROR, payload: t('Please Enroll the Fingerprint before starting the Class !') })
    }
  }
          ...
          <div className="col-12">
            <ul className="row px-0">
              {customerClassesDetails && customerClassesDetails.map((classes, i) => {
                ...
                } else if (!classStartDate && new Date().setHours(0, 0, 0, 0) <= new Date(endDate).setHours(0, 0, 0, 0)) {
                  return (
                    <li key={i} className="d-block col-12 px-0">
                      <div className="row">
