...
class Packages extends Component {
  ...
  handleSubmit() {
    const { t } = this.props
    const { name, amount, period, startDate, endDate, packageId, description, color, image, fromTime, toTime,
      // salesBranches, accessBranches 
    } = this.state
    if (packageId) {
      if (name && amount && period && startDate && endDate && description && color && startDate <= endDate && fromTime && toTime && fromTime < toTime
        // && salesBranches.length && accessBranches.length
      ) {
        const packageInfo = {
  ...
  renderCreatePackageForm() {
    const { t } = this.props
    const { name, amount, period, packageId, description, color, fromTime, toTime,
      // salesBranches, accessBranches 
    } = this.state
    const styles = {
              ...
              <div className="form-control mx-sm-2 inlineFormInputs p-0 border-0 bg-white">
                <div className="d-flex align-items-center h-100">
                  <div style={styles.swatch} onClick={this.handleClick}>
                    <div style={styles.colors} className="d-flex align-items-center justify-content-end" ><span className="iconv1 iconv1-arrow-down font-weight-bold mx-2"></span></div>
                  </div>
              ...
              {this.props.packages.response && getPageWiseData(this.state.pageNumber, this.props.packages.response, this.state.displayNum).map((packages, i) => {
                return (
                  <tr key={i}>
                    ...
                    <td className="dirltrtar">{dateToHHMM(packages.fromTime)}</td>
                    <td className="dirltrtar">{dateToHHMM(packages.toTime)}</td>
                    <td className="text-center">
